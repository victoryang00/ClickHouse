...
public:
    ...
    public:
        static ColumnPtr run(const ColumnsWithTypeAndName & arguments, const DataTypePtr & result_type, size_t input_rows_count)
        {
            ...
            if (col_json_const)
            {
                std::string_view json{reinterpret_cast<const char *>(&chars[0]), static_cast<size_t>(offsets[0] - 1)};
            ...
            for (const auto i : collections::range(0, input_rows_count))
            {
                if (!col_json_const)
                {
                    std::string_view json{reinterpret_cast<const char *>(&chars[offsets[i - 1]]), static_cast<size_t>(offsets[i] - offsets[i - 1] - 1)};
                    document_ok = parser.parse(json, document);
                }
