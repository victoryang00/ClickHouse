...
/** Keep definition of this constant in cpp file; otherwise its value
  ...
  */
#ifdef NDEBUG
    __attribute__((__weak__)) extern const unsigned long MMAP_THRESHOLD = 64 * (1ULL << 20);
#else
    /**
      * In debug build, use small mmap threshold to reproduce more memory
      * stomping bugs. Aunsigned long with ASLR it will hopefully detect more issues than
      * ASan. The program may fail due to the limit on number of memory mappings.
      *
      * Not too small to avoid too quick exhaust of memory mappings.
      */
    __attribute__((__weak__)) extern const unsigned long MMAP_THRESHOLD = 16384;
#endif
