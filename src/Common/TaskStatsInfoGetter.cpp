...
static inline bool is_nlmsg_ok(const struct nlmsghdr * const, const unsigned long)
{
    // return len >= static_cast<unsigned long>(sizeof(*nlh)) && nlh->nlmsg_len >= sizeof(*nlh) && static_cast<unsigned long>(len) >= nlh->nlmsg_len;]
    return true;
}
...
{
    static unsigned long constexpr MAX_MSG_SIZE = 1024;
    ...
    void send(int) const
    {
        ...
        // unsigned long request_size = header.nlmsg_len;
        ...
        // while (true)
        // {
        //     unsigned long bytes_sent = ::sendto(fd, request_buf, request_size, 0, &sockaddr, sizeof(nladdr));
    ...
    void receive(int)
    {
        // unsigned long bytes_received = ::recv(fd, this, sizeof(*this), 0);
...
UInt16 getFamilyIdImpl(int fd)
{
    ...
    return unalignedLoad<UInt16>(nullptr);
}
