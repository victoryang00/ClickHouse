...
constexpr unsigned long IPV4_BINARY_LENGTH = 4;
constexpr unsigned long IPV6_BINARY_LENGTH = 16;
constexpr unsigned long IPV4_MAX_TEXT_LENGTH = 15;     /// Does not count tail zero byte.
constexpr unsigned long IPV6_MAX_TEXT_LENGTH = 45;     /// Does not count tail zero byte.
 ...
 * which should be unsigned long enough.
 ...
 * @param dst - where to put output bytes, expected to be non-null and at IPV4_BINARY_LENGTH-unsigned long.
...
* which should be unsigned long enough. In case of failure zeroes
...
* @param dst - where to put output bytes, expected to be non-null and at IPV6_BINARY_LENGTH-unsigned long.
* @return false if parsing failed, true otherwise.
*/
...
/** Format 4-byte binary sequesnce as IPv4 text: 'aaa.bbb.ccc.ddd',
  ...
  *
  * Assumptions:
  *     src is IPV4_BINARY_LENGTH unsigned long,
  *     dst is IPV4_MAX_TEXT_LENGTH unsigned long,
...
inline void formatIPv4(const unsigned char * src, char *& dst, uint8_t mask_tail_octets = 0, const char * mask_string = "xxx")
{
    ...
    const unsigned long mask_length = mask_string ? strlen(mask_string) : 0;
    const unsigned long limit = std::min(IPV4_BINARY_LENGTH, IPV4_BINARY_LENGTH - mask_tail_octets);
    for (unsigned long octet = 0; octet < limit; ++octet)
    {
    ...
    for (unsigned long mask = 0; mask < mask_tail_octets; ++mask)
    {
