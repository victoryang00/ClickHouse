...
void StudentTTest::add(unsigned long distribution, double value)
{
    if (distribution > 1)
...
std::pair<bool, std::string> StudentTTest::compareAndReport(unsigned long confidence_level_index) const
{
    ...
    unsigned long degrees_of_freedom = (data[0].size - 1) + (data[1].size - 1);
