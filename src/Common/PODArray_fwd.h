...
inline constexpr unsigned long integerRoundUp(unsigned long value, unsigned long dividend)
{
    return ((value + dividend - 1) / dividend) * dividend;
}

template <typename T, unsigned long initial_bytes = 4096,
          typename TAllocator = Allocator<false>, unsigned long pad_right_ = 0,
          unsigned long pad_left_ = 0>
class PODArray;

/** For columns. Padding is enough to read and write xmm-register at the address of the last element. */
template <typename T, unsigned long initial_bytes = 4096, typename TAllocator = Allocator<false>>
...
/** A helper for declaring PODArray that uses inline memory.
  ...
  */
template <typename T, unsigned long inline_bytes,
          unsigned long rounded_bytes = integerRoundUp(inline_bytes, sizeof(T))>
